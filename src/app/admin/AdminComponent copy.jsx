...
export default function AdminPage() {
  ...
  // Создаем profileSummaries
  const todayStr = new Date().toISOString().split('T')[0]

  const profileSummaries = Object.fromEntries(
    Object.values(groupedReports).map(profile => {
      ...
      return [profile[0]?.id, { hasUnverifiedToday, hasUnverifiedEarlier, summary }]
    })
  )
    ...
    <motion.div className="min-h-screen bg-white text-gray-600 p-6">
      ...
      <main className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {uniqueProfiles.map(profile => {
          const { hasUnverifiedToday, hasUnverifiedEarlier, summary } = profileSummaries[profile.id] || { hasUnverifiedToday: false, hasUnverifiedEarlier: false, summary: { calls_sellers: 0, calls_buyers: 0, stickers: 0 } }
          const lastDate = getLastReportDate(profile.id)

          return (
            <Link key={profile.id} href={`/admin?id=${profile.id}`} className={`block rounded-lg p-4 shadow-sm transition hover:shadow-md hover:bg-[#fce7e7] ${hasUnverifiedToday ? 'bg-orange-100' : 'bg-gray-100'}`}>
